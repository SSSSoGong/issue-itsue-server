...
class IssueStatisticsServiceTest {
    ...
    private Project getProject() {
        final Project project = Project.builder().name("new project").build();
    ...
    void 최근_7일간의_날짜별_이슈_발생_횟수_통계를_조회한다() {
        ...
        for (int i = 0; i < todayIssueCount; i++) {
            setUp.add(Issue.builder().project(project).build());
        }
        issueRepository.saveAll(setUp);

        // when
        final List<DateStatisticsResponseDto> dailyStatistics = issueStatisticsService.getDailyStatistics(project.getId(), null);
    ...
    void 최근_6개월간의_월별_이슈_발생_횟수_통계를_조회한다() {
        ...
        // when
        final List<DateStatisticsResponseDto> monthlyStatistics = issueStatisticsService.getMonthlyStatistics(project.getId(), null);
    ...
    void 카테고리별_이슈_통계를_조회한다() {
        ...
        for (int i = 0; i < categories.length; i++) {
            setUp.add(Issue.builder().project(project).category(categories[i]).build());
        }
        issueRepository.saveAll(setUp);

        // when
        final List<CategoryStatisticsResponseDto> categoryStatistics = issueStatisticsService.getCategoryStatistics(project.getId());
    ...
    void 우선순위별_이슈_통계를_조회한다() {
        ...
        for (int i = 0; i < priorities.length; i++) {
            setUp.add(Issue.builder().project(project).priority(priorities[i]).build());
        }
        issueRepository.saveAll(setUp);

        // when
        final List<PriorityStatisticsResponseDto> priorityStatistics = issueStatisticsService.getPriorityStatistics(project.getId());
